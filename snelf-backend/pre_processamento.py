...
async def inicia_pre_processamento(csvFile):
    ...
    df = pd.read_csv(csvFile, usecols=cols, dtype={0: str, 1: int})
    print(df.head())
    ...
    df["DescricaoProduto"] = (
        df["DescricaoProduto"].str.split(pattern, 1).str[0].str.strip()
    )
    df.drop_duplicates(subset=["DescricaoProduto"], inplace=True)
    ...
    async def data_Augmentation():
        os.system(
            'python ./data_augmentation/data_augmentation.py "./teste/medicamentos.csv" "./teste/medicamentos_aumentado.csv" medicamentos 5'
        )
    ...
    def clean_desc(words, desc):
        subs = [
            sub.strip() for sub in re.split("‹|–|::|-|\|", desc)
        ]  # split in substrings
        new_desc = ""
    ...
    for index, row in df.iterrows():
        ...
        if conc == new_conc and qtd == new_qtd:
            df.at[
                index, "descricao"
            ] = new_desc  # replace descricao with cleaned new_desc
        else:
            idxs.append(index)  # storage index for future drop
            removed.append([master_idx, index])
    ...
    df_grouped = (
        df_removed.groupby("master_idx")["removed_idx"].apply(list).reset_index()
    )
    ...
    data_file = "medicamentos_aumentado_preproc.csv"
    df = pd.read_csv(
        "{}{}".format(data_path, data_file), sep=";", dtype={0: int, 1: str, 2: str}
    )
    ...
    dfm.to_csv(
        "{}{}".format(data_path, data_file), sep=";", index=False, encoding="utf-8"
    )
    ...
    X_resampled, y_resampled = ros.fit_resample(
        pd.DataFrame(df[["cod", "descricao"]]), df["chave"]
    )
    ...
    df_oversampled.to_csv(
        data_file, sep=";", header=df.columns, index=False, encoding="utf-8"
    )
